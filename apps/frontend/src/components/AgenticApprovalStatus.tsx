...
interface AgenticApprovalStatusProps {
  workflowId: string;
}
...
export const AgenticApprovalStatus: React.FC<AgenticApprovalStatusProps> = ({ workflowId }) => {
  ...
  useEffect(() => {
    setLoading(true);
    setError(null);
    apiRequest<ApprovalRequest[]>(`/approval/?resource_type=agentic_export&resource_id=${workflowId}`)
    ...
    <div className="mt-4 p-3 bg-background-primary rounded shadow">
      ...
      {approval.status === 'pending' && (
        <ApprovalActions approvalId={approval.id} onAction={() => {
          apiRequest<ApprovalRequest[]>(`/approval/?resource_type=agentic_export&resource_id=${workflowId}`).then((approvals) => setApproval(approvals && approvals.length > 0 ? approvals[0] : null));
        }} />
      )}
