...
export const UserApprovalStatus: React.FC<UserApprovalStatusProps> = ({ userId }) => {
  ...
  useEffect(() => {
    setLoading(true);
    setError(null);
    apiRequest<ApprovalRequest[]>(`/approval/?resource_type=user_export&resource_id=${userId}`)
    ...
    <div className="mt-4 p-3 bg-background-primary rounded shadow">
      ...
      {approval.status === 'pending' && (
        <ApprovalActions approvalId={approval.id} onAction={() => {
          apiRequest<ApprovalRequest[]>(`/approval/?resource_type=user_export&resource_id=${userId}`).then((approvals) => setApproval(approvals && approvals.length > 0 ? approvals[0] : null));
        }} />
      )}
