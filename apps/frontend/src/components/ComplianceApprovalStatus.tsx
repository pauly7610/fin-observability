...
interface ComplianceApprovalStatusProps {
  logId: string;
}
...
export const ComplianceApprovalStatus: React.FC<ComplianceApprovalStatusProps> = ({ logId }) => {
  ...
  useEffect(() => {
    setLoading(true);
    setError(null);
    apiRequest<ApprovalRequest[]>(`/approval/?resource_type=compliance_export&resource_id=${logId}`)
    ...
    <div className="mt-4 p-3 bg-background-primary rounded shadow">
      ...
      {approval.status === 'pending' && (
        <ApprovalActions approvalId={approval.id} onAction={() => {
          apiRequest<ApprovalRequest[]>(`/approval/?resource_type=compliance_export&resource_id=${logId}`).then((approvals) => setApproval(approvals && approvals.length > 0 ? approvals[0] : null));
        }} />
      )}
