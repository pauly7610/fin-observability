...
async def detect_anomalies(
    ...
    with tracer.start_as_current_span("anomaly.detect") as span:
        ...
        try:
            ...
            anomaly_detected_counter.add(sum(anomaly_flags), {"type": "detect", "status": "success", "user": "unknown"})
...
async def get_recent_anomalies(
    ...
    with tracer.start_as_current_span("anomaly.recent_transactions") as span:
        ...
        try:
            recent_anomalies = db.query(Transaction)\
            ...
            # Increment anomaly detected metric
            from apps.backend.main import anomaly_detected_counter
            anomaly_detected_counter.add(len(recent_anomalies), {"type": "recent", "status": "success", "user": "unknown"})
            siem.send_syslog_event(
...
async def get_recent_metric_anomalies(
    ...
    with tracer.start_as_current_span("anomaly.recent_metrics") as span:
        span.set_attribute("limit", limit)
        try:
            recent_metric_anomalies = db.query(SystemMetric)\
            ...
            # Increment anomaly detected metric
            from apps.backend.main import anomaly_detected_counter
            anomaly_detected_counter.add(len(recent_metric_anomalies), {"type": "recent_metric", "status": "success", "user": "unknown"})
            siem.send_syslog_event(
    ...
    with tracer.start_as_current_span("anomaly.train_model") as span:
        ...
        try:
            ...
            if not result.get("success"):
                span.set_status(trace.status.Status(trace.status.StatusCode.ERROR, result.get("reason", "Unknown error")))
            ...
            from apps.backend.main import anomaly_detected_counter
            anomaly_detected_counter.add(1, {"type": "retrain", "status": "success", "user": "unknown"})
