...
def list_exports(
    ...
    if verification_status:
        query = query.filter(ExportMetadataModel.verification_status == verification_status)
    if start_date:
...
def get_export(export_id: int, db: Session = Depends(get_db), user=Depends(require_role(["admin", "compliance"]))) -> ExportMetadata:
...
def batch_verify_exports(
    ...
    for export in exports:
        ...
        export.verification_status = "verified" if hash_ok and sig_ok else "tampered" if not hash_ok else "failed"
...
def redeliver_export(
    ...
    if export.delivery_method and "email" in export.delivery_method and export.file_path:
    ...
    export.delivery_status = "delivered" if email_status and s3_status else "partial" if email_status or s3_status else "failed"
