...
class AuditSummaryService:
    ...
    def summarize_audit(self, logs: List[Dict[str, Any]]) -> dict:
        # Convert logs to a string summary for the agent
        logs_str = "\n".join([str(log) for log in logs])
        prompt = (
            "Given the following audit logs, summarize key events and anomalies using the summarize_audit_logs tool, "
            "and recommend follow-up actions using the recommend_audit_action tool. "
            f"Logs: {logs_str}"
        )
        try:
